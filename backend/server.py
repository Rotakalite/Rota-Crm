...
async def add_bulk_suppliers(
    ...
    try:
        ...
        # 🎯 NEW: Demo limit check for bulk supplier creation
        if not current_user.admin_approved:
            # Check if adding these suppliers would exceed the limit
...
async def get_client_team(client_id: str, current_user: User = Depends(get_current_user)):
    ...
    try:
        # Authorization: Only admins can access team management
        if current_user.role != UserRole.ADMIN:
        ...
        # Get team members (users with parent_client_id = client_id)
        team_members_cursor = db.users.find({
        ...
        for member in team_members:
            formatted_members.append({
                ...
                "name": member["name"],
                "email": member["email"],
                "team_role": member.get("team_role", "staff"),
                "created_at": member["created_at"].isoformat() if isinstance(member["created_at"], datetime) else member["created_at"]
            })
...
async def add_team_member(
    ...
    """Add a new team member to a client"""
    try:
        # Authorization: Only admins can add team members
        if current_user.role != UserRole.ADMIN:
            raise HTTPException(status_code=403, detail="Access denied. Admin role required.")
        ...
        for field in required_fields:
            if field not in team_member_data or not team_member_data[field]:
                raise HTTPException(status_code=400, detail=f"Missing required field: {field}")
        
        # Check if email already exists
        existing_user = await db.users.find_one({"email": team_member_data["email"]})
        if existing_user:
        ...
        try:
            ...
            if clerk_response.status_code not in [200, 201]:
                logging.error(f"Clerk user creation failed: {clerk_response.text}")
            ...
            logging.error(f"Clerk integration error: {clerk_error}")
        ...
        team_member_user = User(
            ...
            client_id=client_id,  # Link to the main client
            parent_client_id=client_id,  # Indicate this is a team member
            is_team_member=True,
            team_role=team_member_data.get("team_role", "staff"),
            admin_approved=True,  # Team members are auto-approved
            created_at=datetime.utcnow(),
            updated_at=datetime.utcnow()
        )
        
        # Save to database
        await db.users.insert_one(team_member_user.dict())
        ...
        try:
            await send_email(
                ...
                html_content=f"""
                <h2>Hoş geldiniz!</h2>
                <p>Merhaba {team_member_data['name']},</p>
                <p><strong>{client.get('hotel_name', 'Hotel')}</strong> için GreenWave CRM takım üyesi olarak eklendiniz.</p>
            ...
            logging.warning(f"Welcome email could not be sent: {email_error}")
        
        return {
            "success": True,
            "message": f"Team member {team_member_data['name']} successfully added",
            "team_member": {
                "id": team_member_user.id,
                "name": team_member_user.name,
                "email": team_member_user.email,
                "team_role": team_member_user.team_role,
                "client_id": client_id
            }
        }
