...
class ClerkAdminManager:
    ...
    async def send_consultant_welcome_email(self, email: str, password: str, consultant_name: str, company_name: str):
        """Send welcome email with credentials to new consultant"""
        try:
            smtp_server = "smtp.gmail.com"
            smtp_port = 587
            sender_email = os.environ.get('GMAIL_USER')
            ...
            # Create message
            message = MIMEMultipart("alternative")
            message["Subject"] = f"🎉 GreenWave CRM Danışman Hesabınız Hazır - {company_name}"
            message["From"] = sender_email
            message["To"] = email
            
            # HTML content for consultant
            html_content = f"""
            <html>
                <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                        <div style="background: linear-gradient(135deg, #10b981, #047857); color: white; padding: 30px; text-align: center; border-radius: 10px;">
                            <h1 style="margin: 0; font-size: 28px;">🌱 GreenWave CRM</h1>
                            <p style="margin: 10px 0 0 0; font-size: 16px;">Sürdürülebilirlik Danışmanlık Sistemi</p>
                        </div>
                        
                        <div style="padding: 30px; background: #f8f9fa; border-radius: 10px; margin-top: 20px;">
                            <h2 style="color: #10b981; margin-bottom: 20px;">Hoş Geldiniz {consultant_name}!</h2>
                            <p><strong>{company_name}</strong> için GreenWave CRM danışman hesabınız başarıyla oluşturuldu.</p>
                            
                            <div style="background: white; padding: 20px; border-radius: 8px; border-left: 4px solid #10b981; margin: 20px 0;">
                                <h3 style="margin-top: 0; color: #047857;">📧 Danışman Giriş Bilgileriniz:</h3>
                                <p><strong>Email:</strong> {email}</p>
                                <p><strong>Şifre:</strong> <code style="background: #e5e7eb; padding: 4px 8px; border-radius: 4px; font-family: monospace;">{password}</code></p>
                            ...
                            <div style="text-align: center; margin: 30px 0;">
                                <a href="https://portal.rotakalitedanismanlik.com" 
                                   style="display: inline-block; background: linear-gradient(135deg, #10b981, #047857); 
                                          color: white; padding: 12px 30px; text-decoration: none; border-radius: 25px; 
                                          font-weight: bold; font-size: 16px;">
                                    🚀 Danışman Paneline Giriş Yap
                                </a>
                            </div>
                            
                            <div style="background: #dbeafe; padding: 15px; border-radius: 8px; margin-top: 20px;">
                                <h4 style="margin-top: 0; color: #1e40af;">🎯 Danışman Olarak Neler Yapabilirsiniz?</h4>
                                <ul style="margin-bottom: 0;">
                                    <li>👥 Müşteri portföyünüzü yönetin</li>
                                    <li>📊 Müşteri tüketim verilerini takip edin</li>
                                    <li>📄 Belge yönetimi ve onayları</li>
                                    <li>🎓 Eğitim programları düzenleyin</li>
                                    <li>📈 Sürdürülebilirlik raporları oluşturun</li>
                                    <li>🤖 AI destekli analiz ve öneriler</li>
                                </ul>
                            </div>
            ...
            logging.info(f"✅ Consultant welcome email sent to {email}")
            return True
            
        except Exception as e:
            logging.error(f"❌ Failed to send consultant welcome email: {str(e)}")
            return False
...
# AI service import - moved to after env loading
...
async def make_user_admin(request: AdminRequest):
    ...
    try:
        ...
        if result.modified_count > 0:
            return {"success": True, "message": f"User {request.email} is now admin"}
        else:
...
async def upload_belge_main_app(
    ...
    try:
        ...
        # Simplified demo limit check - check_demo_limit now handles admin-created clients
...
✅ Bu bir test email'idir. Template'iniz yukarıdaki gibi görünecektir."""
        ...
        await email_service.send_email(
            ...
            <head>
                <meta charset="UTF-8">
                <title>🧪 TEST - {personalized_subject}</title>
                <style>
                    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
                    body {{ 
                        font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; 
            ...
            <body>
                <div class="email-container">
                    <div class="test-banner">
                        <h2>🧪 TEST EMAIL - Gerçek gönderim değil</h2>
                    </div>
                    ...
                    <div class="content">
                        ...
                        <div class="test-info">
                            <h3>📧 Template Test Bilgileri</h3>
                            <div class="test-details">
                                Template: {template_id}<br>
                                Test Tarihi: {datetime.now().strftime('%d.%m.%Y %H:%M')}<br>
                                Test Eden: {current_user.email}
                            </div>
                        </div>
                        ...
                        <div class="warning-box">
                            ...
                            <div class="warning-title">Test Tamamlandı</div>
                            <div class="warning-text">
                                • Bu bir test email'idir, gerçek gönderim değil<br>
                                • Template'iniz yukarıdaki gibi görünecektir<br>
                                • Hazırsanız bulk email gönderimini başlatabilirsiniz
                            </div>
                        </div>
                    </div>
...
async def check_demo_limit(user: User, limit_type: str) -> dict:
    ...
    # 🎯 ENHANCED: Check if client was created by admin (should bypass demo limits)
    ...
    # 🎯 FALLBACK: If user has admin role, bypass demo limits completely
    if fresh_user.get("role") == "admin":
        logging.info(f"✅ User {fresh_user.get('email')} is admin - bypassing demo limits for {limit_type}")
        return {"allowed": True, "message": "Admin user - unlimited access"}
    
    # 🎯 ADDITIONAL CHECK: If no client_id but user was not self-registered, likely admin-created
    if not fresh_user.get("client_id") and not fresh_user.get("self_registered", True):
        logging.info(f"✅ User {fresh_user.get('email')} appears to be admin-created - bypassing demo limits for {limit_type}")
        return {"allowed": True, "message": "Admin-created user - unlimited access"}
...
async def get_pending_approvals(current_user: User = Depends(get_admin_user)):
    ...
    try:
        # Find ONLY users who genuinely need approval 
        # CRITICAL: Only demo users or explicitly pending users
        pending_users = await db.users.find({
            "$and": [
                {"role": "client"},
                {"$or": [
                    # Users who signed up via self-signup (demo users)
                    {"admin_approved": False, "user_status": {"$in": ["pending_approval", "demo"]}},
                    # Explicitly pending users
                    {"user_status": "pending_approval"}
                ]}
            ]
        }).to_list(1000)
...
async def approve_user(user_id: str, current_user: User = Depends(get_admin_user)):
    ...
    try:
        ...
        if user_obj.admin_approved and user_obj.user_status == "approved":
...
async def create_waste_record_via_consumptions(
    ...
    try:
        # 🎯 NEW: Demo limit check for waste data creation
        if not current_user.admin_approved:
            # Check if this client was created by admin (should bypass demo limits)
            if current_user.role == UserRole.CLIENT and current_user.client_id:
                client = await db.clients.find_one({"id": current_user.client_id})
...
async def create_sustainability_target(
    ...
    try:
        # 🎯 NEW: Demo limit check for sustainability target creation
        if not current_user.admin_approved:
            # Check if this client was created by admin (should bypass demo limits)
            if current_user.role == UserRole.CLIENT and current_user.client_id:
                client = await db.clients.find_one({"id": current_user.client_id})
...
async def create_supplier(
    ...
    try:
        # 🎯 NEW: Demo limit check for supplier creation
        if not current_user.admin_approved:
            # Check if this client was created by admin (should bypass demo limits)
            if current_user.role == UserRole.CLIENT and current_user.client_id:
                client = await db.clients.find_one({"id": current_user.client_id})
                if client:
                    ...
                    if creator_user and creator_user.get("admin_approved", False):
                        ...
                        # Regular demo limit check
                        demo_check = await check_demo_limit(current_user, "suppliers")
                        if not demo_check["allowed"]:
                            raise HTTPException(status_code=403, detail=demo_check["message"])
                else:
                    # Client not found, apply demo limits
...
async def add_bulk_suppliers(
    ...
    try:
        ...
        # Demo limit kontrolünü uygula
        if should_apply_demo_limit:
            for _ in request.suppliers_list:
                demo_check = await check_demo_limit(current_user, "suppliers")
                if not demo_check["allowed"]:
                    raise HTTPException(status_code=403, detail=demo_check["message"])
...
# ==========================================
# 📊 SURVEY & CUSTOMER SATISFACTION ENDPOINTS
# ==========================================
...
async def create_survey(
    ...
    """Create a new survey - Admin/Consultant only"""
    if current_user.role not in [UserRole.ADMIN, UserRole.CONSULTANT]:
        raise HTTPException(status_code=403, detail="Access denied. Admin or Consultant role required.")
    
    try:
        # Determine client_id
        if current_user.role == UserRole.ADMIN:
            # Admin can create surveys for any client, but we need client context
            # For now, we'll use a default or let admin specify
            client_id = current_user.client_id or "admin_survey"
        else:
            client_id = current_user.client_id
        ...
        logging.error(f"Survey creation error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_surveys(
    ...
    try:
        ...
        if survey_type:
            filter_query["survey_type"] = survey_type
            
        surveys_cursor = db.surveys.find(filter_query)
...
async def get_public_survey(survey_id: str):
    ...
    try:
        ...
        logging.error(f"Get public survey error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def submit_survey_response(
    ...
    try:
        ...
        if existing_response:
            raise HTTPException(status_code=400, detail="Bu email adresi için zaten yanıt gönderilmiş")
        ...
        return {"message": "Anket yanıtınız başarıyla kaydedildi. Teşekkür ederiz!", "id": response_dict["id"]}
        
    except HTTPException:
        raise
    except Exception as e:
        logging.error(f"Submit survey response error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_survey_responses(
    ...
    """Get survey responses - Admin/Owner only"""
    try:
        # Verify survey exists and permissions
        survey = await db.surveys.find_one({"id": survey_id})
        if not survey:
            raise HTTPException(status_code=404, detail="Survey not found")
        ...
        logging.error(f"Get survey responses error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_survey_campaign(
    ...
    """Create email campaign for survey"""
    try:
        # Verify survey exists and permissions
        survey = await db.surveys.find_one({"id": survey_id})
        if not survey:
        ...
        campaign_dict.update({
            "id": str(uuid.uuid4()),
            "client_id": survey["client_id"],
            "total_sent": len(campaign_data.target_emails),
            "created_at": datetime.utcnow()
        })
        
        await db.survey_campaigns.insert_one(campaign_dict)
        
        return {"message": "Kampanya oluşturuldu", "campaign_id": campaign_dict["id"]}
        
    except HTTPException:
        raise
    except Exception as e:
        logging.error(f"Create survey campaign error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def send_survey_campaign(
    ...
    try:
        ...
        # Check permissions
        if current_user.role != UserRole.ADMIN and campaign["client_id"] != current_user.client_id:
            raise HTTPException(status_code=403, detail="Access denied")
        
        if campaign["is_sent"]:
            raise HTTPException(status_code=400, detail="Campaign already sent")
        
        # Get API URL for survey links
        API_URL = os.environ.get("REACT_APP_BACKEND_URL", "https://rota-crm-production.up.railway.app")
        survey_url = f"{API_URL}/survey/{survey_id}"
        
        # Send emails (basic implementation)
        sent_count = 0
        for email in campaign["target_emails"]:
            try:
                # Create personalized survey link
                personalized_link = f"{survey_url}?email={email}"
                
                # Email content with survey link
                email_html = f"""
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <h2>🌿 {survey['title']}</h2>
                    <p>{campaign['email_content']}</p>
                    
                    <div style="text-align: center; margin: 30px 0;">
                        <a href="{personalized_link}" 
                    ...
                    <p style="font-size: 12px; color: #666;">
                        Bu anket yaklaşık 3-5 dakika sürmektedir.
                    </p>
                </div>
                """
                
                # Send email using our existing email service
                await send_email(
                    to_email=email,
        ...
        return {
            "message": f"Kampanya gönderildi: {sent_count}/{len(campaign['target_emails'])} email başarılı",
            "sent_count": sent_count,
            "total_emails": len(campaign["target_emails"])
        }
        
    except HTTPException:
        raise
    except Exception as e:
        logging.error(f"Send survey campaign error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_survey_analysis(
    ...
    try:
        ...
        if not responses:
            return {"message": "Henüz yanıt bulunmuyor", "total_responses": 0}
        ...
        average_ratings = {}
        rating_questions = [q for q in survey["questions"] if q["question_type"] == "rating"]
        ...
        for question in rating_questions:
            ...
            for response in responses:
                if question["id"] in response["responses"]:
                    try:
                        rating = int(response["responses"][question["id"]])
        ...
        for response in responses:
            for question in survey["questions"]:
                if question["question_type"] == "text" and question["id"] in response["responses"]:
                    text_responses.append(response["responses"][question["id"]])
        
        # Basic recommendations
        recommendations = []
        for question_id, rating_data in average_ratings.items():
            if rating_data["average"] < 3:
                recommendations.append(f"'{rating_data['question']}' konusunda iyileştirme gerekli (Ortalama: {rating_data['average']})")
            elif rating_data["average"] >= 4:
                recommendations.append(f"'{rating_data['question']}' konusunda iyi performans (Ortalama: {rating_data['average']})")
        
        analysis = {
            "survey_id": survey_id,
            "survey_title": survey["title"],
            "total_responses": total_responses,
            "response_rate": "N/A",  # Would need campaign data
...
async def get_hk_daily_report(
    ...
    if is_date_range:
        period_text = f"{start_date_obj.strftime('%d.%m.%Y')} - {end_date_obj.strftime('%d.%m.%Y')}"
    else:
        period_text = start_date_obj.strftime('%d.%m.%Y')
...
async def create_reservation(
    ...
    try:
        ...
        logging.info(f"🔍 Checking room availability for room_id: {reservation_data.room_id}")
        ...
        if conflicting_reservations:
            ...
            logging.warning(f"⚠️ Room {room_number} conflicts found: {conflict_details}")
            raise HTTPException(
                status_code=400, 
                detail=f"Oda {room_number} bu tarihler arasında müsait değil. Mevcut rezervasyon: {', '.join(conflict_details)}"
            )
        
        logging.info(f"✅ Room {room_number} is available for reservation")
...
@api_router.get("/available-rooms-test")
async def available_rooms_test():
    """Simple test endpoint without authentication"""
    return {"status": "working", "message": "Available rooms test endpoint is accessible"}
