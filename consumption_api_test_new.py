...
def get_test_client_id():
    """Get the ID of our test client from the database"""
    client = pymongo.MongoClient(MONGO_URL)
    db = client[DB_NAME]
    
    # Find our test client
    test_client = db.clients.find_one({"hotel_name": "Grand Antalya Resort & Spa"})
...
def get_consumption_id(client_id, year, month):
    """Get a consumption ID from the database"""
    client = pymongo.MongoClient(MONGO_URL)
    db = client[DB_NAME]
    
    consumption = db.consumptions.find_one({
        "client_id": client_id,
        "year": year,
        "month": month
    })
...
class ConsumptionAPITester:
    ...
    def test_consumption_update(self):
        ...
        current_year = datetime.now().year
        consumption_id = get_consumption_id(self.client_id, current_year, 1)
    ...
    def test_consumption_deletion(self):
        ...
        current_year = datetime.now().year
        consumption_id = get_consumption_id(self.client_id, current_year, 7)
