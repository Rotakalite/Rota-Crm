...
async def list_all_users():
    logger.info("Listing all users in both databases")
    ...
    # List all users in sustainable_tourism_crm
    logger.info(f"Users in {DB_NAME}:")
    async for doc in db1.users.find():
        logger.info(f"User: {parse_json(doc)}")
    
    # List all users in rotacrm
    logger.info(f"Users in {ALT_DB_NAME}:")
    async for doc in db2.users.find():
        logger.info(f"User: {parse_json(doc)}")
