    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {showDocumentViewer && viewingDocument && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl m-4 max-w-6xl w-full h-[90vh] flex flex-col">
            {/* Header */}
            <div className="flex justify-between items-center p-4 border-b border-gray-200">
              <div className="flex items-center space-x-3">
                <span className={`text-2xl ${getDocumentType(viewingDocument.original_filename).color}`}>
                  {getDocumentType(viewingDocument.original_filename).icon}
                </span>
                <div>
                  <h2 className="text-xl font-bold text-gray-800">{viewingDocument.document_name}</h2>
                  <p className="text-sm text-gray-500">
                    {viewingDocument.original_filename} • {getDocumentType(viewingDocument.original_filename).type} • 
            ...
            <div className="flex-1 p-4 overflow-hidden">
              {documentViewerLoading ? (
                ...
                <div className="h-full">
                  {isViewableDocument(viewingDocument.original_filename) ? (
                    <div className="h-full">
                      {getFileExtension(viewingDocument.original_filename) === 'pdf' ? (
                        // PDF Viewer
                        <div className="h-full border border-gray-300 rounded-lg overflow-hidden">
                          <iframe
                            src={`${API}/documents/view/${viewingDocument.id}#toolbar=0&navpanes=0&scrollbar=0`}
                            className="w-full h-full"
                            title={`PDF Görüntüleyici - ${viewingDocument.document_name}`}
                          />
                        </div>
                      ) : (
                        // Image Viewer
                        <div className="h-full flex items-center justify-center border border-gray-300 rounded-lg overflow-hidden">
                          <img
                            src={`${API}/documents/view/${viewingDocument.id}`}
                    ...
                    <div className="h-full flex items-center justify-center border border-gray-300 rounded-lg bg-gray-50">
                      <div className="text-center">
                        <div className="text-6xl mb-4">
                          {getDocumentType(viewingDocument.original_filename).icon}
                        </div>
                        <h3 className="text-xl font-semibold text-gray-700 mb-2">
                          {getDocumentType(viewingDocument.original_filename).type} Dosyası
                        </h3>
            ...
            <div className="flex justify-between items-center p-4 border-t border-gray-200 bg-gray-50">
              <div className="flex items-center space-x-4 text-sm text-gray-600">
                <span>📅 Yüklenme: {new Date(viewingDocument.created_at).toLocaleDateString('tr-TR')}</span>
                <span>👤 Yükleyen: {viewingDocument.uploaded_by || 'Sistem'}</span>
                <span>📁 Klasör: {viewingDocument.folder_path || 'Bilinmiyor'}</span>
              </div>
              <div className="flex items-center space-x-3">
                <button
                  onClick={() => downloadDocument(viewingDocument)}
